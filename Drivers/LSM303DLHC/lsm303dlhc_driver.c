...
* Return		: None
*******************************************************************************/
u8_t ReadReg(u8_t deviceAddress, u8_t Reg, u8_t* Data) {
  
  if (HAL_I2C_Mem_Read(pHi2c, deviceAddress, Reg,
...
u8_t ReadRegMult(u8_t deviceAddress, u8_t StartReg, u8_t* Data, u8_t length) {
    
    if (HAL_I2C_Mem_Read(pHi2c, deviceAddress, StartReg,
...
* Return		: None
*******************************************************************************/
u8_t WriteReg(u8_t deviceAddress, u8_t Reg, u8_t Data) {

    if (HAL_I2C_Mem_Write(pHi2c, deviceAddress, Reg,
        I2C_MEMADD_SIZE_8BIT, &Data, 1, HAL_MAX_DELAY) != HAL_OK)
        return MEMS_ERROR;
    else
        return MEMS_SUCCESS;
...
status_t GetAccAxesRaw(AccAxesRaw_t* buff) {
    // TODO: read all registers at once
#if 1
  u8_t valueL;
  u8_t valueH;
...
#else
  if( !ReadRegMult(ACC_I2C_ADDRESS, OUT_X_L_A, (u8_t*)buff, 6) )
      return MEMS_ERROR;
